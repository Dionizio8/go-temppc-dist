...
func TestGetTemperatureUseCase_Ok(t *testing.T) {
	...
	temperatureRepository.On("GetTemperature", ctx, "Limeira").Return(entity.Temperature{Celsius: 20, Fahrenheit: 68, Kelvin: 300}, nil).Once()

	temp, err := usecase.Execute(ctx, "11111111")
...
func TestGetTemperatureUseCase_InvalidZipCode(t *testing.T) {
	...
	_, err := usecase.Execute(ctx, "11111111ABC")
...
func TestGetTemperatureUseCase_AddressNotFound(t *testing.T) {
	...
	addressRepository.On("GetAddress", ctx, "11111111").Return(entity.Address{}, errors.New(entity.ErrAddressNotFoundMsg)).Once()

	_, err := usecase.Execute(ctx, "11111111")
...
func TestGetTemperatureUseCase_TemperatureNotFound(t *testing.T) {
	...
	addressRepository.On("GetAddress", ctx, "11111111").Return(entity.Address{City: "Limeira", State: "SP"}, nil).Once()
	temperatureRepository.On("GetTemperature", ctx, "Limeira").Return(entity.Temperature{}, errors.New("error")).Once()
